...
2. **Run the migrations in order.** Open your project's SQL Editor and run each file in
   `supabase/migrations/` in numeric order (`001_extensions.sql` through `011_storage.sql`) — paste
   the contents of each file and run it before moving to the next. They're idempotent
...
5. **Create an account** from the sign-up tab. A `profiles` row is created for you automatically
   (via the `handle_new_user` trigger from `003_profiles_platforms_deals.sql`).
6. Start adding deals, or use **Import** to upload an Excel/CSV file — `Investment_Import_Template.xlsx`
...
- **Shared/family portfolios (Section 3's own "may be added later")** have their tables
  (`shared_portfolios`, `portfolio_members`) and RLS for those two tables specifically, but no other
  table's RLS policy has been extended with a membership check — a portfolio member cannot yet see
  the owner's deals/payments. Wiring that through is a deliberately separate, larger change (it
  touches every core table's policies).
...
```
supabase/migrations/    11 SQL files, run once in order (see Setup)
web/
